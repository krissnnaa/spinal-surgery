...
def logisticRegressionClassifier(x_train, y_train, x_test, y_test):
    ...
    confusion_matrix_plot(y_test, y_pre,"Logistic Regression")
...
def LinearSVMClassifier(x_train, y_train, x_test, y_test):
    ...
    confusion_matrix_plot(y_test, y_pre,"Linear SVM")
...
def ensembleClassifier(x_train, y_train, x_test, y_test):
    ...
    confusion_matrix_plot(y_test, y_pre,"Random Forest")
...
def decisionTreeClassification(x_train, y_train, x_test, y_test,names):
    ...
    confusion_matrix_plot(y_test, y_pre,"Decision Tree")
...
def adaBoostClassification(x_train, y_train, x_test, y_test):
    from sklearn.ensemble import AdaBoostClassifier
    clf = AdaBoostClassifier(n_estimators=100, random_state=0)
    clf.fit(x_train, y_train)
    y_pre = clf.predict(x_test)
    accuracyScore = clf.score(x_test, y_test)
    print('adaBoost accuracy score for test set=%0.2f' % accuracyScore)
    p, r, f, s = precision_recall_fscore_support(y_test, y_pre, pos_label='Yes', average='binary')
    print('Precision of adaBoost = %f' % p)
    print('Recall of adaBoost = %f' % r)
    print('F1 score of adaBoost = %f' % f)
    confusion_matrix_plot(y_test, y_pre,"Ada Boost")
...
def nnClassifier(x_train, y_train, x_test, y_test):
    ...
    y_pre = clf.predict(x_test)
    accuracyScore = clf.score(x_test, y_test)
    print('MLPClassifier accuracy score for test set=%0.2f' % accuracyScore)
    p, r, f, s = precision_recall_fscore_support(y_test, y_pre, pos_label='Yes', average='binary')
    print('Precision of MLPClassifier = %f' % p)
    print('Recall of MLPClassifier = %f' % r)
    print('F1 score of MLPClassifier = %f' % f)
    confusion_matrix_plot(y_test, y_pre,"MLPClassifier")
...
def deepLearningClassification(x_train, y_train, x_test, y_test):
    ...
    model = Sequential()
    # add model layers
    model.add(Dense(64, activation='relu',input_shape=(249,)))
    ...
    p, r, f, s = precision_recall_fscore_support(y_test, y_pre, pos_label=1.0, average='binary')
    print('Precision of Deep Network = %f' % p)
    print('Recall of Deep Network = %f' % r)
    print('F1 score of Deep Network = %f' % f)
    confusion_matrix_plot(y_test, y_pre,'Deep Learning')


def confusion_matrix_plot(y_true, y_pred, names, normalize=False, cmap=plt.cm.Blues):
    """
    source:
    https://scikit-learn.org/stable/auto_examples/model_selection/plot_confusion_matrix.html
    """
    title = 'Confusion matrix for '+names
    ...
    classes = np.array(['REOPERATION1', 'NOREOPERATION1'])
...
if __name__ == '__main__':
    ...
    # normalize data
    #X_down = normalize(X_down, norm='l1', axis=1)
    ...
    trainLable = np.zeros((len(y_train), 1))
    testLable = np.zeros((len(y_test), 1))
    ...
    #deeo learing
    deepLearningClassification(X_train, trainLable, X_test, testLable)
    ...
    #adaboost
    adaBoostClassification(X_train, y_train, X_test, y_test)
